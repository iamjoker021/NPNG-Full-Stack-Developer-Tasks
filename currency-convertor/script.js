...
const fetchCountryConvertion = async (country) => {
    const response = await fetch(`https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/${country}.json`);
...
const getCountryList = () => {
    ...
    for (const country of Object.values(countries) ) {
        result.push({country_name: country.country_name, currency_code: country.currency_code});
    }
    return result;
}

const createSelectOpt = (text, items) => {
    const select = document.createElement('select');
    select.classList.add('card');
    select.classList.add(`input-${text}`);
    select.required = true;

    const countryList = [{country_name: `Select ${text}`, currency_code: ''},...getCountryList()];
    for (const country of countryList) {
        const opt = document.createElement('option');
        opt.value = country.currency_code;
...
const refreshSelectHidden = (classSelect) => {
    const options = document.querySelector(`${classSelect}`).children;
    for (const opt of options) {
        opt.hidden = false;
    }
}

const createConvertor = () => {
    const convertorCont = document.createElement('form');

    const inp = document.createElement('input');
    inp.classList.add('card');
    inp.placeholder = 'Input value to be converted'
    inp.type = 'number';
    inp.step = 0.01;
    inp.min = 0;
    inp.required = true;
    convertorCont.appendChild(inp);

    const selectFrom = createSelectOpt('from');
    convertorCont.appendChild(selectFrom);

    const selectTo = createSelectOpt('to');
    convertorCont.appendChild(selectTo);

    selectFrom.addEventListener('change', (e) => {
        const selectVal = e.target.value;
        refreshSelectHidden('.input-to');
        const select = document.querySelector(`.input-to option[value="${selectVal}"]`);
        select.hidden = true;
        console.log(selectVal, select);
    })
    selectTo.addEventListener('change', (e) => {
        const selectVal = e.target.value;
        refreshSelectHidden('.input-from');
        const select = document.querySelector(`.input-from option[value="${selectVal}"]`);
        select.hidden = true;
        console.log(selectVal, select);
    })
    ...
    button.type = 'submit';
    convertorCont.appendChild(button);

    const resultPara = document.createElement('p');
    resultPara.classList.add('card');
    convertorCont.appendChild(resultPara);

    convertorCont.addEventListener('submit', async (e) => {
        e.preventDefault();

        const inpVal = parseFloat(document.querySelector('input').value);

        const inp1 = document.querySelector('.input-from').value;
        const inp2 = document.querySelector('.input-to').value;

        const convertionList = await fetchCountryConvertion(inp1);
        const conversion = convertionList[inp2];

        const result = inpVal * conversion;

        console.log(inp1, inp2, inpVal, result);
        resultPara.innerHTML = `Given Input Value: ${inpVal}<br>Input Currency code: ${inp1}<br>Output Currency Code: ${inp2}<br> ConversionRate: ${conversion.toFixed(2)}<br> ${inpVal} * ${conversion.toFixed(2)} = ${result.toFixed(2)}`;
    })

    return convertorCont
}

document.querySelector('body').appendChild(createConvertor())